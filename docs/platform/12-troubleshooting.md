...
- **The application does not follow the desktop's light or dark preference.** The platform follows it only while `Application.RequestedTheme` has never been assigned; assigning it in the `App` constructor is what takes the decision away from the desktop. If nothing is assigning it and the preference is still ignored, check which desktop setting actually feeds the settings portal. On a Cinnamon session that is the XApp portal backend, whose key is `org.x.apps.portal color-scheme`; `org.gnome.desktop.interface color-scheme` has no effect there. `gsettings set org.x.apps.portal color-scheme prefer-light` flips it, and a running application sees the change through `UISettings.ColorValuesChanged`.
- **Placeholder text vanishes after a theme switch and does not come back.** A text control's template reaches its placeholder color through a binding whose fallback is a theme resource, and that fallback does not survive an element-theme change at run time. Set `PlaceholderForeground` on the element itself, to a brush the application owns, as well as re-keying `TextControlPlaceholderForeground`.
